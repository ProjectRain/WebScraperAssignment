...
public final class FileUtility {
     ...
     */
    public static String downloadAndSaveFile(String url, String dest) {
        try {
            URL urlObj = new URL(url);
            //get the name of the file
            String title = urlObj.getFile();
            String outputPath = dest + title;
            try (InputStream inStream = urlObj.openStream();
                    ReadableByteChannel rbcObj = Channels.newChannel(inStream);
                    FileOutputStream fOutStream = new FileOutputStream( outputPath)) {
