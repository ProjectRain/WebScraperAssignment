...
public abstract class GenericLogic< E, T extends GenericDAO<E>>{
    ...
    public void validateString(Map<String, String[]> map, String key, int lenght){
        if(map.containsKey(key) && map.get(key)[0] != null){
            String name = map.get(key)[0];
            if(name.isEmpty() || name.length() > lenght){
                throw new RuntimeException(key+ " must be between 1 and " +lenght+ " char's");
            }
        } else{
